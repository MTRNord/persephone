...
(
    event_id    TEXT    NOT NULL
        CONSTRAINT event_id_unique UNIQUE,
...
BEGIN
    ...
    EXECUTE 'CREATE MATERIALIZED VIEW IF NOT EXISTS ' || quote_ident('room_' || room_cleared) ||
            ' AS SELECT * FROM events WHERE room_id = ' || quote_literal(NEW.room_id);
    EXECUTE 'CREATE UNIQUE INDEX IF NOT EXISTS ' || quote_ident('room_' || room_cleared || '_idx') || ' ON ' ||
...
CREATE TRIGGER tg_room_view_create
    AFTER INSERT
    ON events
    FOR EACH ROW
EXECUTE FUNCTION new_room_view();
...
$$
DECLARE
    state_key_cleared text;
BEGIN
    CASE
        WHEN NEW.state_key IS NOT NULL AND NEW.type = 'm.room.member'
            THEN state_key_cleared := REPLACE(REPLACE(REPLACE(NEW.state_key, '.', '_'), ':', '_'), '@', '');
                 EXECUTE 'CREATE MATERIALIZED VIEW IF NOT EXISTS ' || quote_ident('user_' || state_key_cleared) ||
                         ' AS SELECT * FROM events WHERE type = ''m.room.member'' AND state_key = ' ||
                         quote_literal(NEW.state_key);
                 EXECUTE 'CREATE UNIQUE INDEX IF NOT EXISTS ' || quote_ident('user_' || state_key_cleared || '_idx') ||
                         ' ON ' || quote_ident('user_' || state_key_cleared) || ' (event_id);';
        ELSE
        -- Do nothing
        END CASE;
    RETURN NULL;
...
CREATE TRIGGER tg_user_view_create
    AFTER INSERT
    ON events
    FOR EACH ROW
EXECUTE FUNCTION new_user_view();
